...
Go to the blockchain-js-core directory and build with `npm install`, followed by `tsc`.

Go to the blockchain-js-ui directory and build with `ng build`.

== Run

On node : go to the blockchain-js-core directory and run with `node dist/blockchain-node.js`. This launches a blockchain node on NodeJs. It listens by default on the 9091 TCP port.

To run the browser implementation, go to the blockchain-js-ui directory and run with `ng serve`. You can open as many browser tabs you need on the url `http://localhost:4200`.
